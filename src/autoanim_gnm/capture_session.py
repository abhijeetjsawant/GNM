...
CAPTURE_SESSION_POLICY = (
    "immutable_observation_provenance_with_detector_ingress_hashes_no_claim_inference_v2"
)
LEGACY_CAPTURE_SESSION_SCHEMA_VERSION = "autoanim.capture-session.v1"
LEGACY_CAPTURE_SESSION_POLICY = (
    "immutable_observation_provenance_no_claim_inference"
)
MAX_CAPTURE_SESSION_BYTES = 4 * 1024 * 1024

_LEGACY_VIDEO_ARTIFACT_SCHEMAS = {
...
_VIDEO_ARTIFACT_SCHEMAS = {
    ...
    "pixel_observations": PIXEL_OBSERVATION_SCHEMA_VERSION,
    "observation_v3": OBSERVATION_V3_SCHEMA_VERSION,
    "video_capture_run": VIDEO_CAPTURE_RUN_SCHEMA_VERSION,
    "visual_track": VISUAL_TRACK_SCHEMA_VERSION,
    "visual_track_summary": VISUAL_TRACK_SUMMARY_SCHEMA_VERSION,
}
...
def _artifact_record(
    ...
    path: Path,
    *,
    artifact_schemas: Mapping[str, str] = _VIDEO_ARTIFACT_SCHEMAS,
) -> dict[str, Any]:
    ...
    return {
        ...
        "name": path.name,
        "schema_version": artifact_schemas[logical_name],
        "sha256": _sha256(path),
...
def build_video_capture_session(
    ...
    *,
    ...
    persisted_capture_run = load_video_capture_run(
        artifact_paths["video_capture_run"], expected_capture=capture
    )
    if capture_run is None:
        capture_run = persisted_capture_run
    elif (
        build_video_capture_run_document(capture_run)
        != build_video_capture_run_document(persisted_capture_run)
    ):
        raise ValueError(
            "CaptureSession persisted VideoCaptureRun differs from the expected run"
        )
    build_visual_track(capture, observations, capture_run=capture_run)
    artifacts = [
    ...
    return {
        ...
        "pixel_streams": [
            {
                ...
                "relationship_to_detector_input": (
        ...
        "caveats": [
            ...
            *(
                [
                    "Detector-ingress frame hashes exactly match the independently retained Observation v3 frame hashes; source pixels are not embedded.",
                    "The configured MediaPipe confidence thresholds are retained in the in-memory capture-run envelope and bound into this session.",
                ]
                if same_buffer
                else [
...
def _build_legacy_video_capture_session_v1(
    capture: CaptureTrack,
    observations: PixelObservationTrack,
    *,
    artifact_paths: Mapping[str, str | Path],
) -> dict[str, Any]:
    """Reconstruct the exact historical CaptureSession v1 document.

    This is intentionally private: new jobs must emit v2.  It exists only so
    sealed v1 jobs remain inspectable through the read-only Observation-v3
    reviewer without being upgraded into v2 evidence.
    """
    ...
    for command in (
        ...
        if any(
            ...
            raise ValueError("CaptureSession command recipes must be path-free")
    if set(artifact_paths) != set(_LEGACY_VIDEO_ARTIFACT_SCHEMAS):
        raise ValueError(
            "Legacy CaptureSession artifact set does not match the v1 video schema"
        )
    artifacts = [
        _artifact_record(
            logical_name,
            Path(artifact_paths[logical_name]),
            artifact_schemas=_LEGACY_VIDEO_ARTIFACT_SCHEMAS,
        )
        for logical_name in _LEGACY_VIDEO_ARTIFACT_SCHEMAS
    ]
    return {
        "schema_version": LEGACY_CAPTURE_SESSION_SCHEMA_VERSION,
        "kind": "capture_session",
        "media_kind": "video",
        "policy": LEGACY_CAPTURE_SESSION_POLICY,
        "source_set": {
            "ordering": "single_job_input",
            "sources": [
                {
...
def _read_capture_session_document(path: str | Path) -> dict[str, Any]:
    source = Path(path)
...
def load_verified_legacy_video_capture_session_v1(
    path: str | Path,
    *,
    ...
    """Strictly reconstruct one historical, read-only CaptureSession v1.

    This verifier never synthesizes CaptureRun/VisualTrack evidence and never
    upgrades the returned document or its fail-closed production claims to v2.
    """

    payload = _read_capture_session_document(path)
    if set(artifact_paths) != set(_LEGACY_VIDEO_ARTIFACT_SCHEMAS):
        raise ValueError(
            "Legacy CaptureSession artifact set does not match the v1 video schema"
        )
    if artifact_contracts_preverified:
        ...
        if not _dataclass_values_equal(loaded_capture, expected_capture):
            raise ValueError(
                "Legacy CaptureSession Capture v1 artifact differs from expected capture"
            )
        ...
        if not _dataclass_values_equal(loaded_observations, expected_observations):
            raise ValueError(
                "Legacy CaptureSession pixel observations differ from expected observations"
            )
        loaded_observations.validate_capture(loaded_capture)
        load_verified_capture_jsonl(
    ...
    expected = _build_legacy_video_capture_session_v1(
        loaded_capture,
        loaded_observations,
        artifact_paths=artifact_paths,
    )
    if payload != expected:
        raise ValueError(
            "Legacy CaptureSession does not reconstruct from sealed v1 artifacts"
        )
    if (
        payload.get("schema_version") != LEGACY_CAPTURE_SESSION_SCHEMA_VERSION
        or payload.get("policy") != LEGACY_CAPTURE_SESSION_POLICY
        ...
        raise ValueError("Legacy CaptureSession claims are not fail-closed")
    return payload
...
def load_verified_video_capture_session(
    ...
    *,
    ...
    loaded_capture_run = load_video_capture_run(
        artifact_paths["video_capture_run"],
        expected_capture=loaded_capture,
    )
    if (
        expected_capture_run is not None
        and build_video_capture_run_document(loaded_capture_run)
        != build_video_capture_run_document(expected_capture_run)
    ):
        raise ValueError(
            "CaptureSession persisted VideoCaptureRun differs from the expected run"
        )
    load_verified_visual_track_summary(
        artifact_paths["visual_track_summary"],
        visual_track_path=artifact_paths["visual_track"],
        expected_capture=loaded_capture,
        expected_observations=loaded_observations,
        expected_capture_run=loaded_capture_run,
    )
    expected = build_video_capture_session(
        loaded_capture,
        loaded_observations,
        artifact_paths=artifact_paths,
        capture_run=loaded_capture_run,
    )
    if payload != expected:
