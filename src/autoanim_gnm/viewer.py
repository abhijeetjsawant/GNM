...
function drawDiagnosticOverlay(diagnosticFrame){{
  ...
  for(const regionName of requiredEvidenceRegions){{
    ...
    context.strokeStyle=diagnosticRegionColors[regionName];context.setLineDash(region.qualityState==='diagnostic_clear'?[]:[4,3]);context.strokeRect(x,y,w,h);
...
async function displayExactReviewFrame(index){{
  ...
  }}catch(error){{if(error.name!=='AbortError'||pendingEvidenceFrameIndex===index)failPendingPresentation(`Exact review frame unavailable: ${{error.message}}`)}}
...
async function loadObservationV3Diagnostics(){{
  try{{
    ...
    diagnosticsReady=false;diagnosticFrames=[];evidenceError.hidden=false;evidenceError.textContent=`Pixel diagnostics unavailable: ${{error.message}}`;evidenceDiagnosticState.textContent='Pixel diagnostics rejected · tracker evidence remains available'
  }}
}}
