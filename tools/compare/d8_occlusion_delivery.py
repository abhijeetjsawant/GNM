#!/usr/bin/env python3
"""D8: rebuild the delivery through the REAL build script, into this step's own directory.

Modelled line for line on `tools/compare/d7b_trunk_delivery.py`, which was itself modelled
on `d7_world_vertical_delivery.py`. `scripts/build_commercial_multiview_comparison.py` is
imported as a module and its real `main()` runs, so the detector feed, the association, the
triangulation, the head solve, the toe solve, the sizing, the exporter and the report are
the delivered ones. The only thing that differs between the two runs this file makes is the
state of `src/`.
...
WHY A NEW FILE AND NOT A PARAMETER. `d7b_trunk_delivery.py` carries D7b's clauses and its
committed report is the record of that pass; D7b did not edit D7's file for the same
...
whole point of D8:
  ...
  D7b asserted `triangulated_world_positions_z_up_m` byte-identical across arms, because
  its change lived entirely inside the converter and could not move a landmark. D8's
  conditioning gate, reachability reject and gap clause all sit BETWEEN the raw
  triangulation and the smoothed array, so the smoothed array MUST move -- that is the
  step. `raw_triangulated_world_positions_z_up_m` is captured before any of them and is the
  fixed reference this step keeps (`docs/LADDER_EXECUTION_PLAN.md` section 2, the D8 card:
  "RAW STAYS BIT-IDENTICAL between the D7b and D8 builds and is the one unchanged
  reference; everything downstream loses its byte-identical denominator BY DESIGN").
  Both arrays are reported; only the raw one is a verdict.

TWO ARMS, and the ORDER matters:

  * `--out artifacts/compare/d8-occlusion/delivery-hygiene` run with `src/` UNCHANGED,
    BEFORE any D8 change lands. Every GLB, track and npz must come out byte-identical to
  ...
  * `--out artifacts/compare/d8-occlusion/delivery` run AFTER the change. Its RAW
    triangulated landmarks must still be byte-identical to the shipped ones; its smoothed
    landmarks are expected to differ and the difference is what B4 measures.

    PYTHONPATH=$PWD/src .venv/bin/python tools/compare/d8_occlusion_delivery.py \
        --out artifacts/compare/d8-occlusion/delivery-hygiene --expect-byte-identical
"""
...
def main() -> int:
    ...
    report = {
        "title": ("D8 rebuild -- the real build script, this step's own output directory, "
        ...
        "src_state": ("UNCHANGED (hygiene arm)" if args.expect_byte_identical
                      else "the occlusion repair applied"),
        ...
        "hygiene": {
            ...
            "raw_is_the_fixed_reference": (
                "D8's repair sits AFTER `world` is captured, so the RAW array is the one "
                "array that must not move between the D7b and D8 builds. It is the "
                "denominator every D8 band shares."),
            "smoothed_triangulation_byte_identical": smoothed_identical,
            "smoothed_note": (
                "EXPECTED to be False on the D8 arm and True on the hygiene arm. The "
                "conditioning gate, the reachability reject and the gap clause all act "
                "between the raw array and the smoothed one; if this were True on the D8 "
                "arm the step would have done nothing."),
