#!/usr/bin/env python3
"""D8c: rebuild the delivery through the REAL build script, into this step's own directory.

Modelled line for line on `tools/compare/d8b_length_delivery.py`, which was modelled on
`d8_occlusion_delivery.py`, that on `d7b_trunk_delivery.py` and that on
`d7_world_vertical_delivery.py`. `scripts/build_commercial_multiview_comparison.py` is
...
delivered ones. The only thing that differs between the two runs this file makes is the
state of `src/`.
...
WHY A NEW FILE AND NOT A PARAMETER. `d8b_length_delivery.py` carries D8b's clauses and its
committed report is the record of that pass; D8b did not edit D8's file and D8 did not edit
D7b's, for the same reason. This file differs from D8b's in exactly one place: the shipped
build it compares against is now the **D8b** delivery (`artifacts/commercial-multiview-soma77`
as rebuilt in place by D8b's close-out, 8 of 8 byte-identical), and the row whose effect it
captures is the HIP LINE added to `SEGMENT_LENGTH_RULES`.

  THE SAME-DENOMINATOR CHECK IS ON THE **RAW** ARRAY, NOT THE SMOOTHED ONE.
  The segment-length reject sits between the raw triangulation and the smoothed array --
  ...
  reference this step keeps (`docs/LADDER_EXECUTION_PLAN.md` section 2, the D8c card, band
  B2: "raw array byte-identical"). Both arrays are reported; only the raw one is a verdict.

TWO ARMS, and the ORDER matters:

  * `--out artifacts/compare/d8c-hip/delivery-hygiene` run with `src/` UNCHANGED, BEFORE
    any D8c change lands. Every GLB, track and npz must come out byte-identical to
    `artifacts/commercial-multiview-soma77` (the shipped D8b build). That is the hygiene
  ...
  * `--out artifacts/compare/d8c-hip/delivery` run AFTER the change. Its RAW
...
def main() -> int:
    ...
    report = {
        "title": ("D8c rebuild -- the real build script, this step's own output directory, "
        ...
        "src_state": ("UNCHANGED (hygiene arm)" if args.expect_byte_identical
                      else "the hip-line row added to SEGMENT_LENGTH_RULES"),
        ...
        "hygiene": {
            ...
            "raw_is_the_fixed_reference": (
                "the segment-length reject sits AFTER `world` is captured, so the RAW array is the "
                "one array that must not move between the D9 and D8c builds. It is "
                "the denominator every D8c band shares."),
            "smoothed_triangulation_byte_identical": smoothed_identical,
            "smoothed_note": (
                "EXPECTED to be False on the D8c arm and True on the hygiene arm. The "
                "segment-length reject acts between the raw array and the smoothed "
                "one; if this were True on the D8c arm the step would have done "
                "nothing."),
