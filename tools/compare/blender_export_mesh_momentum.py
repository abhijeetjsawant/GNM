...
for subject in (0, 1):
    ...
    for o in [o for o in bpy.data.objects if o.type == "MESH" and not o.name.lower().startswith("mesh")]:
        bpy.data.objects.remove(o, do_unlink=True)
    meshes = [o for o in bpy.data.objects if o.type == "MESH"]
    if len(meshes) != 1:
        raise SystemExit(f"expected one momentum mesh, found {[m.name for m in meshes]}")
    mesh = meshes[0]
