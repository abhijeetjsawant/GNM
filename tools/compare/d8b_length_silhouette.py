#!/usr/bin/env python3
"""D8b's B1: the photographs, part-wise, ARMS and TORSO. The band the candidate cannot optimise.

INSTRUMENT ONLY. Nothing ships. It is `d9_arm_silhouette.py`'s structure with its two arms
rebound -- the shipped D9 build and this step's D8b rebuild -- and it reuses
...
WHY A NEW FILE AND NOT A PARAMETER. `d9_arm_silhouette.py` carries D9's builds, D9's
...
reason D9 did not edit D8's file, D8 did not edit D7b's and D7b did not edit D7's.

**NOTHING is written under `artifacts/commercial-multiview-soma77/`, `artifacts/compare/i6/`,
`artifacts/compare/d7-pelvis-frame/`, `artifacts/compare/d7b-trunk/`,
`artifacts/compare/d8-occlusion/` or `artifacts/compare/d9-arms/`.** The SAM2 mask cache is
COPIED into this step's own work directory and asserted byte-identical to its source. BOTH
meshes are exported fresh through the REAL Blender path into this step's own directories.

THE DENOMINATOR IS THE RAW ARRAY, and the smoothed one is REPORTED. D9 was converter-only
and could assert both. D8b's reject sits between the raw triangulation and the smoothed
landmarks, exactly where D8's three rules sit, so the smoothed array MUST move -- that is
the step -- and a smoothed array that had NOT moved would mean the step did nothing. The raw
array is captured before the reject and is asserted byte-identical; it is the one reference
both arms share.

TWO CUTS AND TWO PARTS, and the band is all four cells. The window (frame ids 85-125) is
where the captured shoulder line collapses and where the change is expected; the whole take
is reported beside it because a repair that helped the window by hurting everything else
would pass a window-only band. ARMS and TORSO are both banded because the reject withholds
the SHOULDERS, which the torso mesh is bound to as much as the arm mesh is -- D8's own review
found a neck regression its arms-only band could not see.

THE PRE-REGISTERED CLAUSE, verbatim from `docs/LADDER_EXECUTION_PLAN.md` section 2, the D8b
card, band B1, written before this rebuild existed:

    (B1) part-wise silhouette, arms AND torso, window and whole take, not worse on either
    performer with the CI clear on the D7b/D8 predicate, improvement predicted on
    performer 1's window

BLIND TO: depth; a left/right mirror of a fore-aft symmetric pose; and, within each part,
where inside the outline a limb sits -- an arm can move centimetres inside its own silhouette
without moving a pixel. A folded-arm control does not realise "a limb hidden inside the
outline" under four cameras (CLAUDE.md), so this band is paired with B3's placement figures
and B2's bit-identity and is never quoted alone. Every figure is read against a mesh bound to
an asset whose proportions are not the performer's, so it scores a CHANGE against a large
standing shape mismatch.

    PYTHONPATH=$PWD/src .venv/bin/python tools/compare/d8b_length_silhouette.py

Writes `artifacts/compare/d8b-length/silhouette-partwise.json` and caches every per-frame
...
BUILDS = (
    ("D9", ROOT / "artifacts/commercial-multiview-soma77", OUT_DIR / "silhouette-work-d9"),
    ("D8b", OUT_DIR / "delivery", OUT_DIR / "silhouette-work-d8b"),
)
...
PREREGISTERED = {
    "source": ("docs/LADDER_EXECUTION_PLAN.md section 2, the D8b card, band B1, committed "
               "before this rebuild existed"),
    "arms_and_torso_not_worse_than_D9": (
        "whole take AND the window, ARMS and TORSO, on EITHER performer, with the CI clear "
        "on the D7b/D8 predicate: the upper bound of the 95 % interval on the paired "
        "difference is at or above 0."),
    "improvement_predicted_on_performer_1s_window": (
        "the captured shoulder line on the falling performer collapses to 122-274 mm "
        "against his own 364 on frames 110-122, and the delivered shoulders, arms and "
        "torso follow it. Withholding those frames should put the mesh back over the "
        "person."),
    "mamma_mesh_oracle": "BIT-IDENTICAL between runs -- it reads none of our track.",
    "what_this_cannot_settle": (
        "the silhouette scores the MESH, and a limb can move inside its own outline "
        "without moving a pixel. It also cannot say whether a WITHHELD frame was recovered "
        "correctly -- only whether the mesh covers more of the person. Paired with B3's "
        "placement figures against the RAW captured points and B2's bit-identity, never "
        "quoted alone."),
}
...
def main() -> int:
    ...
    raw_identical = all(np.array_equal(raw[label], raw["D9"], equal_nan=True)
                        for label in labels)
    smoothed_identical = all(np.array_equal(smoothed[label], smoothed["D9"])
                             for label in labels)
    ...
    subject_to_body = mamma_index_for(smoothed["D9"])
    ...
    for s in (0, 1):
        ...
        for part in ("torso", "arm", "whole"):
            ...
            for name, mask_frames in cuts.items():
                ...
                cell[f"{part}_D8b_minus_D9"] = pw.paired(
                    series[part]["D8b"], series[part]["D9"], mask_frames, draws)
        ...
        for part in ("arm", "torso"):
            for cut in ("whole_take", "window"):
                cells[(part, cut)] = row["cuts"][cut][f"{part}_D8b_minus_D9"]
        ...
        for (part, cut), cell in cells.items():
            subject_verdicts[f"clause_{part}_{cut}_not_worse_than_D9"] = {
                ...
                "improvement_predicted_here": bool(s == 1 and cut == "window"),
                "rose_with_ci_clear_of_zero": rose(cell)}
        subject_verdicts["reported_arms_outside_the_window"] = row["cuts"][
            "outside_window"]["arm_D8b_minus_D9"]
        subject_verdicts["reported_torso_outside_the_window"] = row["cuts"][
            "outside_window"]["torso_D8b_minus_D9"]
