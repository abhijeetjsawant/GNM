...
|---|---|---|---|
| **D1 facing** | fix the rig's mirrored left/right naming so `_frame_alignment` stops yielding a 180° yaw | 11 | forward-dot against footage (`camera_overlay.py` flip check) on every camera · I6 silhouette on front-back-asymmetric frames · a mirrored self-consistent human must fail the forward-dot even where IoU passes |
