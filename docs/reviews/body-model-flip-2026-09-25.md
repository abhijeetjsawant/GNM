...
> **Disposition (coordinator, 2026-09-25, Astra's merge round):** FAIL (oracle). This review and its records are merged to
> main ALONE. The candidate (the default flip, the roster tooling, the schema-aware silhouette and verifier, the build's
> refusals, the bootstrap, and the coordinator's post_merge.sh and ladder.py) is pinned at tag `ladder/D4i-fail-b073253`.
> The default on main stays `rig`. D4i-b re-registers the flip from that tag.
