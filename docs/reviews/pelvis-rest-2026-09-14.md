...
| **every stage's source, in order** (round 9) | all seven named stage reports present by identity, the pre-change arm earliest | each stage read from its OWN record — it used to be read from hygiene's duplicated copy of everyone else's times, so moving the tripwire's own value one second earlier passed. One stage (`silhouette-partwise.json`) carries a **genuine** build-time stamp, whose head commit git places after the src change; six are retrospective, and a retrospective stamp must name the log its time came from, which the gate re-stats | **PASS** |
...
functions, so the fuzz and the gate cannot disagree about what a leaf is: 762 labels, 189
